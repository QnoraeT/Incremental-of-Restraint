    ...
    if (tmp.tab === -1) {
        ...
        if (tmp.statTab === 1) {
            ...
            for (let i = 0; i < tmp.factors.points.length; i++) {
                txt += `<span>${tmp.factors.points[i]}</span>`;
            }
            ...
            for (let i = 0; i < tmp.factors.prestige.length; i++) {
                txt += `<span>${tmp.factors.prestige[i]}</span>`;
            }
            ...
            if (hasSetbackUpgrade('b1')) {
                ...
                for (let i = 0; i < tmp.factors.prestigeEssence.length; i++) {
                    txt += `<span>${tmp.factors.prestigeEssence[i]}</span>`;
                }
            ...
            if (hasSetbackUpgrade(`r10`)) {
                ...
                for (let i = 0; i < tmp.factors.genXP.length; i++) {
                    txt += `<span>${tmp.factors.genXP[i]}</span>`;
                }
                txt += `<b style="font-size: 14px">Final: ${format(tmp.generatorFeatures.gain)}</b>`;
                html['genXPFactors'].setHTML(txt);
            }

            html['genEnhFactors'].setDisplay(hasSetbackUpgrade(`r10`));
            if (hasSetbackUpgrade(`r10`)) {
                txt = `<b style="font-size: 14px">Generator Enhancer Gain</b>`;
                for (let i = 0; i < tmp.factors.genEnh.length; i++) {
                    txt += `<span>${tmp.factors.genEnh[i]}</span>`;
                }
            ...
            if (Decimal.gte(tmp.generatorFeatures.genEnhBuyables[2].eff, 1)) {
                ...
                for (let i = 0; i < tmp.factors.tier.length; i++) {
                    txt += `<span>${tmp.factors.tier[i]}</span>`;
                }
            ...
            if (Decimal.gte(player.ascendUpgrades[13], 3)) {
                ...
                for (let i = 0; i < tmp.factors.tier1Time.length; i++) {
                    txt += `<span>${tmp.factors.tier1Time[i]}</span>`;
                }
